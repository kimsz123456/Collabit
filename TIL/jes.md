...
# [02/04] 파이썬에서 MongoDB에 저장한 주관식 설문 결과 읽어오기
최근 타입이 있는 자바를 주로 쓰다보니 오랜만에 파이썬을 마주하고 당황스러워졌다.
파이썬에서 생성한 데이터를 몽고디비에 저장하고 이걸 다시 스프링에서 읽어오는 API가 필요한 상황이었다.
이에 따라 MongoRepository를 extends해서 사용했었는데 주관식 설문 결과를 못 읽어오는 상황이 발생했다.
문제를 해결하기 위해 Repository 대신에 MongoTemplate을 이용한 조회로 바꾸었다.
그래도 잘 되지 않아서 혹시 몰라 ProjectInfoCode를 String으로로 바꾸었더니 해결이 되었다.
숫자를 등록했지만 그게 String으로 들어갔다는 점에서 파이썬의 타입 없음에서 오는 문제라는 생각이 들었다.
그 결과 다행히 조회에 성공했고 데이터를 잘 받아왔다.
그 뒤 messages에 저장된 내용을 다시 DTO에 담아 리스트로 만들려고 하는데 첫 번째 메시지만 들어오고 이후 메시지가 들어오지 않았다.
배열 형태로 오지 않고 객체들만 쭉 들어와서 앞뒤로 [] 괄호를 붙여서 해결하였다.
파이썬에 타입이 없는 부분이 왜 불편한지 이해할 수 있었다.
